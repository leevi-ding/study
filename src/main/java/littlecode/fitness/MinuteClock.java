...
public class MinuteClock extends JFrame implements ActionListener, ItemListener {

    final int interval = 1000 * 60;

    boolean looping = false; //是否循环播放
    String[] choices = {"../../music/dingdong.wav", "../../music/success.wav"}; //声音文件名数组
    URL file1 = getClass().getResource(choices[0]); //声音文件1
    URL file2 = getClass().getResource(choices[1]); //声音文件2
    ...
    JComboBox jcbFiles = new JComboBox(choices); //文件选择组合框
...
//            chosenClip.loop(); //循环播放选择的声音剪辑对象
            int temp_interval = 0;
            try {
                temp_interval = Integer.parseInt(jTextField.getText().trim()) * 1000;
            } catch (Exception e) {
                temp_interval = this.interval;
            }
            int finalTemp_interval = temp_interval;
            new Thread(() -> {
                while (looping) {
                    chosenClip.play(); //循环播放选择的声音剪辑对象
                    try {
                        Thread.sleep(finalTemp_interval);
    ...
    public static void main(String s[]) {
        new MinuteClock(); //创建AudioPlayDemo对象
    }
}